<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

class CreateReceptionConfirmationMaquiladosTable extends Migration
{
    /**
     * Run the migrations.
     *
     * @return void
     */
    public function up()
    {
        Schema::create('reception_confirmation_maquilados', function (Blueprint $table) {
            $table->id();
            $table->string('code_order');
            $table->integer('odoo_product_id');
            $table->integer('quantity_maquilada');
            $table->integer('decrease')->nullable();
            $table->integer('product_clean')->nullable();
            $table->integer('observations')->nullable();
            $table->timestamps();
        });
    }

    /**
     * Reverse the migrations.
     *
     * @return void
     */
    public function down()
    {
        Schema::dropIfExists('reception_confirmation_maquilados');
    }
}
